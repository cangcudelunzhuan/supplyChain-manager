...
@connect(({ payRec, loading }) => ({
  payRec,
  detailLoading: loading.effects['payRec/FeeDetail'],
}))
@Form.create()
class Index extends Component {
  ...
  toex = async (data, key, e) => {
    ...
    let res = await this.props.dispatch({
      type: key === 1 ? 'payRec/FeeExamine' : 'payRec/FeeConfrim',
      payload: {
    ...
    return (
      ...
      >
        <Spin spinning={detailLoading}>
          <DyAuditLog
            id={this.props.match.params.id} type={23}
          />
          <Form {...formItemLayout}>
            <Row gutter={gutter}>
            ...
            <Row gutter={gutter}>
              ...
              {renderField(getFieldDecorator, '银行账户', 'payeeBankCardNo', <Input disabled />)}
            </Row>
            <Row gutter={gutter}>
            ...
            <Row gutter={gutter}>
              ...
              {this.state.examineStatus === 2 && (
                <Fragment>
                  ...
                  />, {
                    rules: [{ required: true, message: '请选择赎回日期!' }],
                  })}
