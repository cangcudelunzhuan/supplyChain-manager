...
@connect(({ payRec, loading }) => ({
  payRec,
  detailLoading: loading.effects['payRec/RepaymentDetail'],
}))
@Form.create()
class Index extends Component {
  ...
  toex = async (data, key, e)=>{
    ...
    let res = await this.props.dispatch({
      type: key===1?'payRec/RepaymentExamine':'payRec/RepaymentConfrim',
      payload: {
    ...
    return (
      ...
      >
        <Spin spinning={detailLoading}>
          <DyAuditLog
            id={this.props.match.params.id} type={21}
          />
          <Form {...formItemLayout}>
            <Row gutter={gutter}>
            ...
            <Row gutter={gutter}>
              <div className="block-title">收款方信息</div>
              {
                this.state.initProps.payWay===2?(
                  <Fragment>
                    {renderField(getFieldDecorator, '持牌机构', 'licenseOrganizationName', <Input disabled/>)}
                    {renderField(getFieldDecorator, '账户类型', 'tickerPayAccountType', <Select placeholder="请选择" disabled>
                      {accountType.map((item)=>{
                        return (<Select.Option  value={item.id} key={item.id}>{item.name}</Select.Option>);
                      })}
                    </Select>)}
                    {renderField(getFieldDecorator, '银行名称', 'tickerPayBankName', <Input disabled/>)}
                    {renderField(getFieldDecorator, '银行开户支行', 'tickerPayBankBranchName', <Input disabled/>)}
                    {renderField(getFieldDecorator, '银行账号', 'tickerPayBankCardNo', <Input disabled/>)}
                  </Fragment>
                ):(
                  <Fragment>
              ...
              {this.state.examineStatus===2&&(
                <Fragment>
                  {renderField(getFieldDecorator, '还款日期', 'date', <DatePicker format="YYYY-MM-DD" disabled={edit}
                    disabledDate={(e)=>{ return e < moment().subtract(1, 'year'); }}
                  />, {
                    rules: [{ required: true, message: '请选择起息日!' }],
                  })}
                  {renderField(getFieldDecorator, '附件', 'fileList',
                    <DyUpload app={this} onChanges={(fileName, fileList)=>{ fileChange(fileName, fileList, this) ; }}
                      fileList={this.state.fileList}
                      action={'global/toUpload'}
                      filename= {'fileList'} status={edit}
                    />, {
                      rules: [
