...
@connect(({ licensee, loading }) => ({
  licensee,
  tableLoading: loading.effects['licensee/getDetail'],
}))
@Form.create()
    ...
    return (
      ...
      >
        <Spin spinning={tableLoading}>
          <Form {...formItemLayout} >
            <Row gutter={gutter}>
              <div className="block-title">授信额度详情</div>
              {renderField(getFieldDecorator, '持牌机构名称', 'xxxxxx', <Input disabled />, )}
              {renderField(getFieldDecorator, '社会统一信用代码', 'xxxxxxx', <Input disabled />, )}
              {renderField(getFieldDecorator, '出资机构名称', 'xxxxx', <Input disabled />, )}
              {renderField(getFieldDecorator, '社会统一信用代码', 'xxxxxxx', <Input disabled />, )}
              {renderField(getFieldDecorator, '授信额度（元）', 'xxxxxxx', <Input disabled />, )}
              ...
              { renderField(getFieldDecorator, '授信类型', 'xxxxxxxxx', <Select placeholder="请选择授信类型" >
                {creditType.map((item) => {
                  return (<Select.Option value={item.id} key={item.id}>{item.name}</Select.Option>);
                })}
              </Select>
