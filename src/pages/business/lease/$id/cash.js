...
class Detail extends Component {
  ...
  getRelaInfo = async () => {
    ...
    if (id && id.length === 10) {
      ...
      let riskPremiumRate = this.props.form.getFieldValue('riskPremiumRate');
      this.props.form.setFieldsValue({
        ...res.data,
        ...
        rentRate: format.formatPoint(res.data.financeRate + riskPremiumRate),
      });
    }
  }
  // 输入内容改变清空出资机构相关信息
  ...
  checkMoney = () => {
    ...
    if (totalGoodsValue * (100 - advanceRate) / 100 > dealerAvailableAmount || totalGoodsValue * (100 - advanceRate) / 100 > fundingAvailableAmount) { // 增加判断是否大于授信可用额度 1
      ...
      let tradeAmount = format.formatPoint(totalGoodsValue / 100 * (100 - advanceRate));
  ...
  tradeChange = () => {
    ...
    let maxAmount = format.formatPoint(totalGoodsValue * (100 - advanceRate) / 100);
    let riskDepositRate = this.props.form.getFieldValue('riskDepositRate');
    if (tradeAmount > maxAmount) {
      message.error(`融资金额额度最大值为${maxAmount}`);
      this.props.form.setFieldsValue({
        'tradeAmount': maxAmount,
        'advanceRate': format.formatPoint((1 - (maxAmount / totalGoodsValue)) * 100),
        'advanceAmount': format.formatPoint(totalGoodsValue - maxAmount),
        'riskDepositAmount': maxAmount * riskDepositRate / 100,
      });
    } else {
      this.props.form.setFieldsValue({
        'advanceAmount': format.formatPoint(totalGoodsValue - tradeAmount),
        'advanceRate': format.formatPoint((1 - (tradeAmount / totalGoodsValue)) * 100),
        'riskDepositAmount': format.formatPoint(tradeAmount * riskDepositRate / 100),
      });
    }
  }
          ...
          <Spin spinning={this.props.tableLoading}>
            <Form {...formItemLayout} onSubmit={this.handleSubmit}>
              ...
              <Row gutter={gutter}>
                ...
                {renderField(getFieldDecorator, '计息方式', 'interestType', <Select onChange={this.rentTypeChange} placeholder="请选择计息方式" disabled>
              ...
              <Row gutter={gutter}>
                ...
                {isFixed === 1 && renderField(getFieldDecorator, '固定付费日', 'payDay', <Input disabled={isDisabled} placeholder="不能填写28日之后的日期" />, {
                  rules: [
                    { required: true, ...RegExp.payDay },
                  ],
                })}
                {renderField(getFieldDecorator, '融资金额（元）', 'tradeAmount', <Input onBlur={this.tradeChange} disabled={isDisabled || isEditDisabled} />, {
                  rules: [
